...
function Danger({ message , alertVisibilityCheck }) {
    const [visibility, setVisibility] = useState(false);

    useEffect(() => {
        alertVisibilityCheck && setVisibility(true);
        const timer = setTimeout(() => {
            setVisibility(false);
        }, 5000);

        return () => clearTimeout(timer); // Cleanup the timer on unmount
    }, [alertVisibilityCheck]);
        ...
        <div className='fixed right-10 bottom-10'>
            {
                visibility ? (
                    <div id="toast-danger"
                        ...
                        <div className="ml-3 text-sm font-normal">{message || "Something Went wrong"}</div>
                        <button
                            type="button"
